...
import { CloudUploadIcon, Trash } from "@hugeicons/core-free-icons";

const DROPZONE_VARIANTS = {
  base: "relative rounded-md p-4 flex justify-center items-center flex-col cursor-pointer min-h-[150px] min-w-[200px] border-2 border-dashed border-border hover:border-muted-foreground transition-colors duration-200 ease-in-out",
  image: "border-0 p-0 min-h-0 min-w-0 relative bg-muted shadow-md",
  active: "border-primary",
  disabled: "bg-muted/50 border-border/70 cursor-default pointer-events-none",
...
>(({ dropzoneOptions, width, height, className, disabled, showProgress, ...props }, ref) => {
    ...
    <div className="flex flex-col items-center relative">
      ...
      >
        ...
        {displayUrl ? (
          ...
          >
            <HugeiconsIcon className="mb-1 h-7 w-7" icon={CloudUploadIcon} />
            <div className="font-medium">Drag an image or Click to upload</div>
            {maxSize && (
        ...
        {showProgress && displayUrl && fileState?.status === "UPLOADING" && (
          ...
          fileState.status !== "COMPLETE" && (
            ...
            >
              {fileState.status === "UPLOADING" ? (
                ...
                <HugeiconsIcon className="block h-4 w-4 text-muted-foreground" icon={Trash} />
              )}
            </button>
          )}
      </div>
